...
public class IncrementalUpdater {
	 ...
	 * @return
	 */
	CoreExpr applyEdit(CoreExpr expr, final int editOffset, final int editLength, final int editStartLine, final int editStartColumn, final String replacement, final String newSourceCode) {
		/*
		...
		int lineDeltaTemp=0;
		final int offsetDelta = replacement.length();
		...
		final ExprTransformer transformer = new ExprTransformer() {
			...
			final CoreExprVisitor<Expr> visitor = new CoreExprVisitor<Expr>() {
				...
				CoreExpr reparse(CoreExpr e) {
					int nodeStartOffset = adjustOffset(e.getFileRange().getStartOffset());
					int nodeEndOffset = adjustOffset(e.getFileRange().getEndOffset());
				...
				private <T extends HasFileRange> boolean editTouchesStartOrEndOfNode(Option<T> optExpr) {
					if(optExpr.isSome()) {
				...
				private boolean editTouchesStartOrEndOfNode(HasFileRange child) {
					...
					return (editOffset <= range.getStartOffset() && endOffset >= range.getStartOffset()) ||
						   (editOffset <= range.getEndOffset() && endOffset >= range.getEndOffset());
				}
				...
				private boolean touchesEdit(CoreExpr child) {
					FileRange range = child.getFileRange();
					return endOffset >= range.getStartOffset() && editOffset <= range.getEndOffset();
				}
			...
			private int adjustLine(int line) {
				return line >= editStartLine ? line + lineDelta : line;
			}
			private int adjustColumn(int line, int column) {
				return line == editStartLine ? column + columnDelta : column;
			}
			private int adjustOffset(int offset) {
				return offset >= editOffset ? offset + offsetDelta : offset;
			}
			private FilePos adjustFilePos(FilePos pos) {
				return editOffset < pos.getOffset() ? pos : 
					new FilePos(adjustOffset(pos.getOffset()),
							    adjustLine(pos.getLine()),
							    adjustColumn(pos.getLine(), pos.getColumn()));
			}
			@Override
			public FileRange transform(FileRange range) {
				...
				if(editOffset > range.getEndOffset()) {
					return range;
				}
				
				// Create a new range by adjusting the offsets / lines
				return new FileRange(range.getFilename(),
						adjustFilePos(range.getStart()),
						adjustFilePos(range.getEnd()));
			}
		};
		return (CoreExpr) expr.transform(transformer);
	}
}
